td<-as.Date(w.tdays('2018-03-27','2018-10-31')$Data[[1]])
...
update_factor<-function(ed){
  ...
  if(length(f_hk[is.na(f_return)]$date)!=0){
    ssd<-w.tdaysoffset(-569,ed)
    temp_hk<-data.table(dbGetQuery(con,str_c('select f_seccode, date, f_return from factor_return where f_seccode= \'',LG300.WI,'\' and date >= \'',ssd, '\'',sep = '')))
