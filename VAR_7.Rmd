...
if (Cache.equity == T){
  ...
  # Cache to SQL
    con <- dbConnect(MySQL(),host="localhost",dbname="database",user="root",password="llh07310150")
...
if (read.ratio == T){
  ...
  temp.equity.ratio.df = data.frame(date = equity.hold$date, name = equity.hold$name, SEC_NAME = equity.hold$secname, seccode = equity.hold$seccode, ratio = equity.hold$ratio)
  equity.ratio.df = temp.equity.ratio.df[temp.equity.ratio.df$date %in% date.list,]  
  ...
  if (!"ratio_table" %in% table.name.list){
    dbWriteTable(con,"ratio_table", equity.ratio.df, overwrite = TRUE)
  }else{
...
equity.ratio.df <- dbGetQuery(con,"select * from ratio_table;")
dbDisconnect(con)
### Parameters needed 
date.list = unique(equity.ratio.df$date)
...
H_share_indexes_length = length(c(hs.index))
all_indexes = c(basic.index,sw.industry.indexes,hs.index)
all_indexes_length = length(all_indexes)    
fund.name.list <- unique(equity.ratio.df$name)
## Data set made up
index_name = w.wss(all_indexes,'SEC_NAME')$Data  # Cache index name data from WIND
equity_name = w.wss(total.ticker.list,'SEC_NAME')$Data # Cache equity name data from WIND
...
date.intersec = intersect(index_day_list, original_A_equity_data$date) # Find the period inetersection for ratio and equity (index)
...
# Period selection (select recent 570 days if it is enough, or take as long as it can)
index_dt_final = as.data.table(index_dt_final)
selected_index_dt_final = index_dt_final[, .SD[which(date.list == data_start_day): length(date.list)], by = seccode]
...
# for (name.number in 1:1){
  ...
  # Delete this part?
  temp.equity.ratio.df = equity.ratio.df[equity.ratio.df$date==date.list[length(unique(date.list))]&equity.ratio.df$name==fund.name.list[name.number]&equity.ratio.df$seccode!="166015",] # get the last days' ratio
  sum.equity.ratio.df = as.data.table(temp.equity.ratio.df)[, sum(ratio),by = seccode]
  unique.equity.ratio.df = unique(as.data.table(temp.equity.ratio.df),by="seccode")
  unique.equity.ratio.df$ratio = sum.equity.ratio.df$V1
  modified.equity.ratio.df = unique.equity.ratio.df
  setorder(modified.equity.ratio.df, -ratio)
  # end
  ticker.list = modified.equity.ratio.df$seccode #  ??? Why there is an auto-setting order after setorder
  equity_total_ratio = as.data.table(modified.equity.ratio.df)[name==fund.name.list[name.number],sum(ratio)]
  if (length(modified.equity.ratio.df$ratio)>=10){
    Top.10.weight = sum(modified.equity.ratio.df$ratio[1:10]) # if not enough 10?
  } else{
    Top.10.weight = sum(modified.equity.ratio.df$ratio[1:length(modified.equity.ratio.df$ratio)]) 
  }
  ...
  equity_ticker_list = as.character(temp_list$equity_ticker_list) # This list is shorter than the "ticker.list",
  equity_length_ticker = temp_list$equity_length_ticker
  data_equity = temp_list$data_equity
  final_data_equity =temp_list$final_data_equity # Do we need all days? No
  setorder(final_data_equity, seccode, date)

  # Filling data (EM or Index)
  # applied.date.list = sort(unique(final_data_equity$date))
  applied.date.list = sort(unique(final_data_equity$date))
  EM.temp.list= EM.Missing.data.process(final_data_equity,review_period,equity_ticker_list,equity_length_ticker, applied.date.list) # Check HK equity regression
...
for (day.to.view in 1:process_period){
  ...
  # sort(unique(equity.ratio.df$date))
  temp.equity.ratio.df = equity.ratio.df[equity.ratio.df$date==day.to.view.in.list & equity.ratio.df$name==fund.name.list[name.number] & equity.ratio.df$seccode!="166015",]
  # equity.ratio.df[equity.ratio.df$date==day.to.view.in.list,]
  if (dim(temp.equity.ratio.df)[1] ==0){
  ...
  unique.equity.ratio.df = unique(as.data.table(temp.equity.ratio.df),by="seccode")
  unique.equity.ratio.df$ratio = sum.equity.ratio.df$V1
  modified.equity.ratio.df = unique.equity.ratio.df # hold ratio
  setorder(modified.equity.ratio.df, -ratio)
  ticker.list = modified.equity.ratio.df$seccode 
  ...
  ratio_list = merge(equity_dt_final_select_day, modified.equity.ratio.df[,c("seccode","ratio")], by.x = "seccode", by.y = "seccode", all.x = TRUE)$ratio
  port_rt_in_selected_day = equity_dt_final_select_day$logreturn_T0 %*% t(t(ratio_list))
  port_rt_list = append(port_rt_list,port_rt_in_selected_day)  # The true return list everyday 
  
  # Select period for equities
  ...
  applied.days = applied.all.days[c(day.to.view.in.list.index-total_days+1):day.to.view.in.list.index]
  selected_applied_equity_data = as.data.table(selected_equity_data)[, .SD[c(day.to.view.in.list.index-total_days+1):day.to.view.in.list.index],by = seccode]
  final_data_equity_copy = as.data.table(selected_applied_equity_data)
  
  # Select period for indexes
  # Period selection (select recent 570 days if it is enough, or take as long as it can)
  selected_index_dt_final = index_dt_final[, .SD[c(day.to.view.in.list.index-total_days+1): day.to.view.in.list.index], by = seccode]
  ...
  temp_list = regression(process_period, final_data, full_equity_df, final_data_equity_copy)
  m_equity_ticker_list = temp_list$m_equity_ticker_list
  m_equity_length_name = temp_list$m_equity_length_name
  coef_df = temp_list$coef_df
...
for (day.to.view in 1:process_period){
  ...
  modified.equity.ratio.df = unique.equity.ratio.df # hold ratio
  setorder(modified.equity.ratio.df, -ratio)
  # Today = as.Date(unique(modified.equity.ratio.df[modified.equity.ratio.df$name==fund.name.list[name.number],]$date))
...
# }
  ...
  # Omega series
  temp_equity_ticker_list <- as.character(equity_dt_final_select_day$seccode)
    # dim(modified.equity.ratio.df[ticker.list %in% temp_equity_ticker_list,])[1]
  temp_weight <- ratio_list
  temp_equity_ticker_list_length = length(temp_equity_ticker_list)
  selected_equity_data = modified2_original_equity_data[modified2_original_equity_data$seccode %in% temp_equity_ticker_list,]
  setorder(selected_equity_data, seccode, date)
  ...
  final_full_index_df = sapply(1:c(length(full_index_df)), function(x) if(x==1 | x > length(A_share_indexes_length)){full_index_df[,x] = full_index_df[,x]}else{full_index_df[,x] = full_index_df[,x] - full_index_df[,1]}) # Calculate excess earnings
  ...
  lines(x,-rev(var_list),col="green")
  lines(x,-rev(var_list)*2.33/1.65,col="darkgreen")
# }
```
...
equity.logr.calculation <- function(modified2_original_equity_data){
  ...
  # ticker.list[!ticker.list %in% unique(modified2_original_equity_data$seccode)]
  ...
  final_data_equity <- as.data.table(data_equity)[, .SD[c(length(temp_date_list)-process_period+1):length(temp_date_list)],by = seccode]
...
EM.Missing.data.process <- function(final_data_equity, review_period, equity_ticker_list, equity_length_ticker, applied.date.list){
  ...
  # applied.date.list = applied.days
  # 
  ...
  IPO.Date.list = temp.ipo.df[!is.na(temp.ipo.df$ipo_date),]$ipo_date # H equities no ipo date
  IPO.Date.list
  ...
  # final_data_equity[final_data_equity$seccode == "300618.SZ",]
  invalid_days_to_count = c()
  ticker.to.delete = c()
  
  # Do something for A share new equities
  equity_ticker_list_without_HK = equity_ticker_list[!str_detect(equity_ticker_list,"HK")]  # Delete HK. equities to match the "IPO.Date.list" 
  for (i in 1:length(IPO.Date.list)){
    # i = 10
    if (!all(is.na(temp_equity_df[,as.character(equity_ticker_list_without_HK[i])]))){
      # if (!all(temp_equity_df[,equity_ticker_list[i]]==0)){# boundary condition, stop all the time NA and HK share
        all.profit = temp_equity_df[!is.na(temp_equity_df[,equity_ticker_list_without_HK[i]]),][,equity_ticker_list_without_HK[i]] # find new equities all profits
        logic = c(all.profit == 0)
        if (any(logic)){
          if (as.factor(IPO.Date.list[i]) %in% applied.date.list){
            # ipo.date.index = min(which(!is.na(temp_equity_df[,equity_ticker_list[i]]))) # find the day the equity goes pubish
            ipo.date.index = min(which(!temp_equity_df[,equity_ticker_list_without_HK[i]]==0))
            ...
            temp_equity_logr_df[,equity_ticker_list_without_HK[i]][1:c(begin.to.count.date-1)] = NA # filiing the value with EM or index return
            invalid_days_to_count = append(invalid_days_to_count,c(begin.to.count.date-1))
            ticker.to.delete = append(ticker.to.delete, equity_ticker_list_without_HK[i])
          }
        } else{
          temp_equity_logr_df[,equity_ticker_list_without_HK[i]] = NA
        }
    }
  ...
  for (ticker in equity_ticker_list){
    ...
    if (missing.total > length(unique(Missing.Data.cache$date))/2){ #!!!!!!!! The boundary condition setting, "process_period" appropriate? No
  ...
  ### Using Amelia
  amelia.out <- amelia(Missing.Data.df, m = 1) # "m" is the number of imputed datasets to create
  amelia.imputed <- amelia.out$imputations[[1]]
...
regression <- function(process_period, final_data, amelia.inputed, final_data_equity){
  # regression(process_period, final_data, amelia.imputed, final_data_equity)
  ...
  # Regression progress
  final_index_df = sapply(1:c(length(index_df)), function(x) if(x==1 | x>length(A_share_indexes_length)){index_df[,x] = index_df[,x]}else{index_df[,x] = index_df[,x] - index_df[,1]}) # Calculate excess earnings
...
VaR_Calculation <- function(process_period, all_indexes_length, coef_df){
  ...
  # Replace process
  # weight
  weight_temp_list = modified.equity.ratio.df[modified.equity.ratio.df$seccode %in% m_equity_ticker_list,][,c("seccode","ratio")]
